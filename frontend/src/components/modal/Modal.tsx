...
export default function Modal () {
  const {setShowEditModal} = useModalContext()
    ...
    <section className="relative flex flex-col gap-5 rounded-xl min-w-52 bg-white p-5">
      ...
      <form className="flex flex-col gap-5" onSubmit={handleSubmit}>
        ...
        <div className="flex flex-col gap-2">
          ...
          <textarea name="description" id="description" className="outline-none bg-gray-100 rounded-lg min-h-32 p-2 resize-none" placeholder="Add description" onChange={handleDescriptionValue} value={descriptionValue} />
        </div>

        <div className="flex flex-row justify-end gap-5">
          <button onClick={handleClick}> Cancel </button>
          <button className="bg-blue-400 py-2 px-3 text-white rounded-full"> Edit </button>
        </div>
      </form>
    </section>
  )
